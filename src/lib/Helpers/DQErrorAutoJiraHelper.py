...
class DQErrorAutoJiraHelper(AbstractAMSZabbixWrapperHelper):
    def __init__(self):
    ...
    def set_parameters(self, full_filename, dq_error_txt):
        ...
        labels = 'Data_Quality,' + self.project.strip() + '_' + file_type_str
        ...
        self.jibbix.comm_status_prod = '{}|Not Normal'.format(self.hostname)
        self.jibbix.assignee = self.assignee
        self.jibbix.watchers = self._get_watchers()
        self.jibbix.summary = '[DQ ERROR: {}] {} | {}'.format(self.hostname, os.path.basename(full_filename), file_type_str)
