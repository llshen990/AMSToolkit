...
class AMSSignalFileAbsentDependencyCheck(AbstractAMSDependencyCheck):
    """
    ...
    def instructions_for_verification(self):
        ret_str = 'There should not be a file matching the below command:%s' % os.linesep
        ret_str += 'ls -la %s' % (os.path.join(self.sig_dir, self.AMSDependencyChecker.dependency))
        return ret_str

    def commandline_output(self):
        return 'The below signal file should be absent but it is present:{}{}'.format(os.linesep, os.path.join(self.sig_dir, self.AMSDependencyChecker.dependency))