...
class AMSSTPHealthCheckCompleteHandler(AbstractCompleteHandler):
    """
    ...
    def _run_complete_handler(self, schedule, is_success):
        ...
        try:
            ams_config = AMSConfig(self.AMSConfig.AMSDefaults.default_environment_config)
            ...
            stp = locate('Toolkit.Models.STP')(ams_config)
        ...
        add_comment_jibbix_options.description += "STPHealthCheck "

        if result.is_success():
            result.subject = 'STPHealthCheck successful'
            add_comment_jibbix_options.description += "is successful and services are UP."
        else:
            result.subject = "STPHeathCheck failed"
    ...
    def instructions_for_verification(self):
        ret_str = 'See the following for triage: %s%s%sVerify the STPHealthCheck failure by running the following as the run user:%s%s/sso/sfw/ghusps-toolkit/toolkit_venv/bin/python /sso/sfw/ghusps-toolkit/ams-toolkit/src/Toolkit/Controllers/stp.py --config_file=%s' % (self.AMSConfig.AMSDefaults.default_stp_jira_link ,os.linesep, os.linesep, os.linesep, os.linesep, self.AMSConfig.AMSDefaults.default_environment_config)
        return ret_str
