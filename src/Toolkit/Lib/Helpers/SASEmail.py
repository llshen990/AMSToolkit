...
from Toolkit.Lib.Helpers import StripHtmlTags, Text2Html
...
class SASEmail(object):
    ...
    def __init__(self):
        ...
        self.set_from(AMSDefaults().from_address)
    ...
    def set_subject(self, subject):
        ...
        elif len(subject) > AMSDefaults().max_mail_subject_length:
            self.full_subject = subject
            self.logger.warning('SASEmail requires subject be <= 78 characters to follow RFC-2822 standards, truncating.')
            subject = subject[:AMSDefaults().max_mail_subject_length-3] + '...'
    ...
    def _create_message(self):
        ...
        try:
            ...
            if self.full_subject:
                self.msg.add_header('Full Subject', self.full_subject)
    ...
    def send(self):
        ...
        try:
            try:
                domainname = subprocess.check_output(['dnsdomainname']).strip()
                mailhost = subprocess.check_output(['dig', domainname, 'MX', '+short']).strip().rstrip('.').split()[1]
            except Exception as e:
                errors.append(e)
            self.smtp = smtplib.SMTP(mailhost)
        except Exception as e:
            errors.append(e)
            self.logger.warning('Unable to use {} for smtp, will look for alternates in /etc/mail.rc'.format(mailhost))
            try:
                if os.path.exists('/etc/mail.rc'):
                    with open('/etc/mail.rc', 'r') as mailfile:
                        mailserver = re.search('(set smtp=smtp://)(.*)', mailfile.read())
                    self.smtp = smtplib.SMTP(mailserver.group(2))
                else:
                    raise
            except Exception as e:
                errors.append(e)
            ...
            # This flows somewhat awkwardly but I don't think we should change it until we get full test coverage of this function
            try:
                self._create_message()
            except Exception as e:
                errors.append(e)
                self.logger.error('Unable to determine valid smtp server, we have encountered these errors:\n{}'.format(str(errors)))
            if self.smtp:
                self.smtp.quit()
            else:
                self.logger.warning('SMTP not quit because was None')
        return True
